def test_failure_duplicates(scripter):
    ...
    result = (scripter / "examples/script-failure-0.py").run(["--help"])
...
def test_failure_missing(scripter):
    ...
    result = (scripter / "examples/script-failure-1.py").run(["--help"])
    assert (
...
def test_failure_wrong_setup(scripter):
    ...
    result = (scripter / "examples/script-failure-2.py").run(["--help"])
    assert (
        "click.plus.base.ExtensionDevelopmentError: "
