...
def testdir(datadir, request) -> Path:
    from conftest import __file__  # type: ignore

    result = (
        Path(request.module.__file__).relative_to(Path(__file__).parent).with_suffix("")
    )
...
def pytest_addoption(parser):
    ...
    group.addoption(
        ...
        type=Path,
        help="""
        source data: the datadir fixture will be a path rooted under DATADIR.
        resolution order is:
